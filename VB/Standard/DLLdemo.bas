...
'  PHLIB.DLL v.2.3 from Visual Basic
...
Dim Counts(0 To HISTCHAN-1) As Long
Dim i As Long
...
SyncDivider = 8         'see PicoHarp manual
...
Retcode = PH_ClearHistMem(Dev(0), 0) 'always block 0
If Retcode < 0 Then
...
Retcode = PH_GetBlock(Dev(0), Counts(1), 0) 'must pass the first array element
    If Retcode < 0 Then
...
ConsolePrint "Integralcount = " & CStr(Integralcount) & vbCrLf
...
Open "DLLDEMO.OUT" For Output Shared As #1
For i = 0 To HISTCHAN-1
    Print #1, CStr(Counts(i))
Next i
Close
