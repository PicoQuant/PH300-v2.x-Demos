...
'
'  TTTRMODE.bas
'  A demo how to use the PicoHarp 300 programming library
'  PHLIB.DLL v.2.3 from Visual Basic in TTTR Mode
'  The program uses a text console for user input/output
'  Hardware settings are hard coded, see: 'change as required'
'
...
'''''D E C L A R A T I O N S for THLIB.DLL-access '''''''''''''
...
Private Declare Function PH_SetPHR800Input Lib "phlib.dll" (ByVal devidx As Long, ByVal level As Long, ByVal edge As Long) As Long
...
Dim buffer(0 To TTREADMAX-1) As Long
Dim i As Long
Dim j As Long
Dim FiFoWasFull As Boolean
Dim CTCDone As Boolean
Dim Blocksz As Long
...
ConsolePrint "PicoHarp 300  TTTR mode demo" & vbCrLf
...
Retcode = PH_Initialize(Dev(0), MODE_T2) '0=histogramming, 2=T2Mode, 3=T3Mode
If Retcode < 0 Then
...
If Retcode < 0 Then
    ConsolePrint "GetHardwareVersion error " & CStr(Retcode) & vbCrLf
    GoTo Ex
End If
...
Blocksz = 32768         'max TTREADMAX, use larger value for high count rates
...
Do
    ...
    Retcode = PH_TTReadData(Dev(0), buffer(1), Blocksz) 'may return less!
    If (Retcode < 0) Then
        ConsolePrint " ReadData error" & vbCrLf
        GoTo stoptttr
    End If

    If (Retcode > 0) Then
        For i = 1 To Retcode    ' save data in buffer to binary file
            Put #1, , buffer(i) ' doing this in a loop is terribly slow but VB
        Next i                  ' doesn't allow a block put with variable length
        Progress = Progress + Retcode
    ...
    'Countrate0 = PH_GetCountRate(0); 'can be called here if needed but costs time
    'Countrate1 = PH_GetCountRate(1); 'can be called here if needed but costs time
Loop
