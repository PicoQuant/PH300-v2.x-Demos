...
program routing;
//{$apptype console}	  //need this for Delphi but not for Lazarus

uses
  SysUtils;

const
  {constants, see PHDEFIN.H and ERRCODES.H}
  LIBVERSION='2.3';
...
var
  ...
  Routermodel:array[0..7] of char;
  Routerversion:array[0..7] of char;   
...
begin
  writeln;
  writeln('PicoHarp 300 PHLib    Routing Demo              PicoQuant GmbH, 2009');
  writeln('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
  ...
  if retcode<0 then
  begin
    writeln('PH_GetRouterVersion failed. Aborted.');
    goto ex;
  end
  else
  ...
  while(cmd<>'q') do
    begin
      ...
      for i:=0 to 3 do  {fetch histograms of all 4 channels}
        ...
        writeln('Total count in channel ', i, ' = ',Integralcount);
        end;

      flags:=PH_GetFlags(dev[0]);
      if flags<0 then
      begin
        writeln('PH_GetFlags failed. Aborted.');
        goto ex;
      end;
