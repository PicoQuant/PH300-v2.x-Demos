...
% a PHR 800 you muast also suitably set its inputs and CFDs (if installed).
%
...
ret = calllib('PHlib', 'PH_GetRoutingChannels', dev(1));
if(ret<0)
...
for (i=0:3) % must fetch 4 Blocks for 4-channel Routing
    ...
    [ret,countsbuffer] = calllib('PHlib', 'PH_GetBlock', dev(1), bufferptr, i); 
    if (ret<0)
        fprintf('\nPH_GetBlock error %ld. Aborted.\n', ret);
        closedev;
        return;
    end;
